...
#define DUMP_SLOT(slot, name) ({ \
	...
	DUMP_SLOT(PS_GL_PIPE_CACHE, "  - Pre Cache");
	DUMP_SLOT(PS_GL_PIPE_CACHE_INDICES, "    - Fetch indices");
	DUMP_SLOT(PS_GL_PIPE_CACHE_CHECK, "    - Check");
	DUMP_SLOT(PS_GL_PIPE_PC, "  - Post Cache");
	DUMP_SLOT(PS_GL_PIPE_PC_FETCH, "    - Fetch vertices");
	DUMP_SLOT(PS_GL_PIPE_PC_PRE_CULL, "    - Pre Cull");
